...
from infer_owlvit_image_guided import load_model_and_processor
...
def render_detections(
    ...
    query_box: tuple[int, int, int, int] | None,
    scores,
    boxes,
) -> Image.Image:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run OWL-ViT image-guided detection on all images in a folder.")
    parser.add_argument("--input-dir", required=True, help="Folder containing target images.")
    parser.add_argument("--output-dir", required=True, help="Folder to save rendered output images.")
    ...
    parser.add_argument(
        "--query-source-image",
        help="Optional source image to crop from when using --query-box. Defaults to each target image.",
    )
    parser.add_argument(
        "--query-box",
        type=int,
    ...
    for image_path in image_paths:
        ...
        query_image, query_box = load_batch_query_image(
            args.query_image,
            args.query_source_image,
            args.query_box,
            target_image,
        )
        ...
        print(f"Saved {output_path} ({len(results['boxes'])} detections)")
