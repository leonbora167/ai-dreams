#!/usr/bin/env python3
"""CLI entry point for Modular Sequential Multi-Camera (MTMC) Tracker."""
import argparse
import sys
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Modular Sequential Multi-Camera (MTMC) Tracker"
    )
    parser.add_argument(
    ...
    print(f"Starting Modular Sequential MTMC Tracker:")
    print(f"  Config: {args.config}")
    print(f"  Videos: {len(videos)} stream(s) in {video_dir}")
    print(f"  Target: {cfg['system']['output_dir']}\n")
