...
def latest_artifact_dir(root: Path) -> Path:
    candidates = [p for p in root.glob("panns_cnn14_*") if p.is_dir()]
    if not candidates:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run inference with a trained PANNs Cnn14 model.")
...
def main() -> None:
    ...
    model = Cnn14(num_classes=len(metadata["label_names"]), mel_bins=metadata["n_mels"])
